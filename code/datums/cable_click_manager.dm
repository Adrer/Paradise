/datum/cable_click_manager
	...
	/// Mouse catcher
	var/atom/movable/screen/fullscreen/stretch/cursor_catcher/catcher
...
/datum/cable_click_manager/proc/set_user(mob/new_user)
	...
	if(user)
		UnregisterSignal(user, list(COMSIG_MOB_LOGIN, COMSIG_MOB_LOGOUT, COMSIG_MOB_SWAPPING_HANDS, COMSIG_MOB_CLICKON))
...
/datum/cable_click_manager/proc/create_catcher()
	...
	catcher = user.overlay_fullscreen("cable_laying", /atom/movable/screen/fullscreen/stretch/cursor_catcher/cable, 0)
...
/datum/cable_click_manager/proc/on_catcher_click(atom/source, location, control, params)
	...
	if(params[RIGHT_CLICK])
		...
		disable_catcher()
		to_chat(usr, "<span class='notice'>Advanced wire placement disabled.</span>")
		return
	...
	if(!isfloorturf(T) || !T.can_have_cabling())
		to_chat(user, "<span class='warning'>[T] can not have cables.</span>")
		clear_state()
		return
...
/datum/cable_click_manager/proc/intercept_click(datum/source, atom/A, params)
	...
	if(catcher.mouse_opacity != MOUSE_OPACITY_OPAQUE)
		enable_catcher()
		to_chat(usr, "<span class='notice'>Advanced wire placement enabled.</span>")
