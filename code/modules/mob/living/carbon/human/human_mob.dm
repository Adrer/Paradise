...
/mob/living/carbon/human/proc/check_and_regenerate_organs(mob/living/carbon/human/H) //Regenerates missing limbs/organs.
	...
	var/list/species_organs = H.dna.species.initial(has_organ) //Compile a list of species organs and tack on the mutantears afterward.
	if(H.dna.species.mutantears)
		species_organs["ears"] = H.dna.species.mutantears
	for(var/obj/item/organ/internal/target_organ in species_organs)
		if(!(organ in types_of_int_organs)) //If the mob is missing this particular organ...
			var/target_organ_slot = target_organ.initial(slot)
			if(H.get_organ_slot(target_organ_slot)) //Check to see if the user already has an organ in the slot the 'missing organ' belongs to. If they do, skip implantation.
				...
				var/replacement_organ = new organ(H) //Create the organ inside the player.
				replacement_organ.insert(H)
